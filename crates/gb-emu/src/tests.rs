...
use crate::io::interrupt::{INT_TIMER, INT_VBLANK, Interrupt};
...
fn test_boot_rom_shadowed_by_cartridge_after_disable() {
    ...
    assert_eq!(
        mmu.get8(0x0000),
        0x31,
        "Boot ROM byte 0 must be 0x31 when active"
    );
...
fn test_call_and_ret() {
    ...
    assert_eq!(
        cpu.get_pc(),
        0xC000,
        "After CALL, PC must point to subroutine"
    );
    assert_eq!(
        cpu.get_sp(),
        0xFFFC,
        "CALL must push 2 bytes, moving SP down"
    );
    assert_eq!(
        mmu.get16(0xFFFC),
        0xD003,
        "Return address must be instruction after CALL"
    );
...
fn test_halt_wakes_on_interrupt_even_with_ime_clear() {
    ...
    assert!(
        !cpu.halted,
        "Halt must clear when interrupt is pending, even with IME=0"
    );
}
...
fn test_ppu_vblank_interrupt_fires_at_scanline_144() {
    ...
    assert!(
        vblank_fired,
        "VBlank interrupt must fire after 144 scanlines ({} cycles)",
        total_cycles
    );
...
fn test_ppu_oam_dma_copies_to_oam() {
    ...
    assert_eq!(
        ppu.get_oam(0xFE00),
        0x10,
        "OAM byte 0 (Y) must match DMA source"
    );
    assert_eq!(
        ppu.get_oam(0xFE01),
        0x08,
        "OAM byte 1 (X) must match DMA source"
    );
    assert_eq!(
        ppu.get_oam(0xFE02),
        0x42,
        "OAM byte 2 (tile) must match DMA source"
    );
}
