...
impl Ppu {
    ...
    fn render_background(
        &self,
        color_index: &mut [u8; SCREEN_WIDTH],
        bg_opaque: &mut [bool; SCREEN_WIDTH],
    ) {
        let map_base = if self.lcdc & LCDC_BG_MAP != 0 {
            0x1C00
        } else {
            0x1800
        };
    ...
    fn render_window(
        ...
        let map_base = if self.lcdc & LCDC_WINDOW_MAP != 0 {
            0x1C00
        } else {
            0x1800
        };
...
impl MemHandler for Ppu {
    ...
    fn on_write(&mut self, addr: u16, value: u8) -> MemWrite {
        match addr {
            ...
            // The actual copy is performed in desktop.rs where the MMU is accessible.
            0xFF46 => {
